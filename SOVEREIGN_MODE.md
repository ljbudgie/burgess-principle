...
4. Click **Verify integrity** and confirm Iris recomputes the Merkle root without errors.
5. Click **Full system integrity check** and confirm Memory Palace + trigger ledger continuity pass together.
6. Export the latest memory receipt and confirm it contains the signed entry, signed root, and Merkle inclusion proof.
7. Bring the network down, queue a hub push, then reconnect and flush the queue.
8. In Chrome DevTools → **Application** inspect:
    - `memoryEntries`
    - `memoryRoots`
    - `memoryReceipts`
    - `hubSyncQueue`
    - `hubAudit`
9. On iOS PWA, confirm the Memory Palace still works in the foreground and the UI explains that background reliability is lower.
